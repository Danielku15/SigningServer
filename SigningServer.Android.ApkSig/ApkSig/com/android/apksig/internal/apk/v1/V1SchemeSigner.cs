...
namespace SigningServer.Android.Com.Android.Apksig.Internal.Apk.V1
{
    ...
    public abstract class V1SchemeSigner
    {
        ...
        public static string GetSafeSignerName(string name)
        {
            ...
            for (int i = 0;i < global::System.Math.Min(nameCharsUpperCase.Length, 8);i++)
            {
        ...
        {
            ManifestParser.Section mainAttributes = null;
            if (sourceManifestBytes != null)
            {
                try
                {
                    var sourceManifest = new ManifestParser(sourceManifestBytes);
                    mainAttributes = sourceManifest.ReadSection();
                }
            ...
            {
                foreach (var attribute in mainAttributes.GetAttributes())
                {
                    mainAttrs.PutValue(attribute.GetName(), attribute.GetValue());
                }
            }
            else 
            {
