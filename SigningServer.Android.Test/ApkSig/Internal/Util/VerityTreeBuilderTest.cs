...
namespace SigningServer.Android.Test.ApkSig.Internal.Util
{
    ...
    {
        [TestMethod]
        public void SHA256RootHashMatch()
        {
        ...
        private static String generateRootHash(String inputResource, byte[] salt)
        {
            byte[] input = Resources.toByteArray("internal.util." + inputResource);
            assertNotNull(input);
            using (VerityTreeBuilder builder = new VerityTreeBuilder(salt))
            {
                return TestHelpers.encodeHex(builder.generateVerityTreeRootHash(
        ...
        {
            return DataSources.asDataSource(ByteBuffer.wrap(Encoding.UTF8.GetBytes(data)));
        }

        [TestMethod]
        public void generateVerityTreeRootHashFromPlaceholderDataSource()
        {
            ...
            using (VerityTreeBuilder builder = new VerityTreeBuilder(null))
            {
                ...
                assertEquals(expectedRootHash, TestHelpers.encodeHex(rootHash));
            }
