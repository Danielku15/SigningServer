 ...
 */
     ...
     */
        protected abstract CloseableWithDataSource createDataSource(byte[] contents);

        protected CloseableWithDataSource createDataSource(String contents)
        {
            return createDataSource(Encoding.UTF8.GetBytes(contents));
        }

        [TestMethod]
        public void testSize()
        {
            using (CloseableWithDataSource c = createDataSource("Hello12345"))
            {
        ...
        {
            using (CloseableWithDataSource c = createDataSource("test1234"))
            {
                DataSource ds = c.getDataSource();
        ...
        {
            using (CloseableWithDataSource c = createDataSource("abcdefghijklmnop"))
            {
                DataSource ds = c.getDataSource();
        ...
        protected static void assertSliceEquals(
            String expectedContents, DataSource ds, long offset, int size)
        {
...

        {
...

            {
                ...
                (IndexOutOfRangeException expected)
            {
            }
        }

        protected static void assertFeedEquals(
            String expectedFedContents, DataSource ds, long offset, int size)
        ...
        private static void assertCopyToThrowsBufferOverflow(
            DataSource ds, long offset, int size, ByteBuffer buf)
     ...
     */
        public static String toString(ByteBuffer buf)
        {
            ...
            return Encoding.UTF8.GetString(arr, offset, size);
        }

        public class CloseableWithDataSource : IDisposable
        {
            private readonly DataSource mDataSource;
            private readonly IDisposable mCloseable;

            private CloseableWithDataSource(DataSource dataSource, IDisposable closeable)
            {
            ...
            public static CloseableWithDataSource of(DataSource dataSource, IDisposable closeable)
            {
        ...
        private sealed class NullDataSink : DataSink
        {
            public static readonly NullDataSink INSTANCE = new NullDataSink();
