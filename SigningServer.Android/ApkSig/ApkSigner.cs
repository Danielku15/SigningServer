 ...
 */
     ...
     */
        private static readonly short ALIGNMENT_ZIP_EXTRA_DATA_FIELD_HEADER_ID = unchecked((short)0xd935);

        /**
     * Minimum size (in bytes) of the extensible data block/field used for alignment of uncompressed
     * entries.
     */
        private static readonly short ALIGNMENT_ZIP_EXTRA_DATA_FIELD_MIN_SIZE_BYTES = 6;

        private static readonly short ANDROID_COMMON_PAGE_ALIGNMENT_BYTES = 4096;

        /** Name of the Android manifest ZIP entry in APKs. */
        private static readonly String ANDROID_MANIFEST_ZIP_ENTRY_NAME = "AndroidManifest.xml";
        ...
        private readonly bool mForceSourceStampOverwrite;
        ...
        private readonly bool mV4SigningEnabled;
        private readonly bool mVerityEnabled;
        private readonly bool mV4ErrorReportingEnabled;
        ...
        private readonly ApkSignerEngine mSignerEngine;

        private readonly FileInfo mInputApkFile;
        private readonly DataSource mInputApkDataSource;

        private readonly FileInfo mOutputApkFile;
        private readonly DataSink mOutputApkDataSink;
        private readonly DataSource mOutputApkDataSource;
        ...
        private ApkSigner(
            ...
            bool forceSourceStampOverwrite,
            int? minSdkVersion,
     ...
     * @throws InvalidOperationException if this signer's configuration is missing required information
     ...
     */
...

        {
            ...
            try
            {
                ...
                {
                    throw new InvalidOperationException("Input APK not specified");
                }

                IDisposable @out = null;
                try
                {
                    ...
                    {
                        throw new InvalidOperationException("Output APK not specified");
                    }
...

        {
            ...
            List<Hints.ByteRange> pinByteRanges = pinPatterns == null ? null : new List<Hints.ByteRange>();
            ...
            else
            {
                ...
                List<DefaultApkSignerEngine.SignerConfig> engineSignerConfigs =
                    new List<DefaultApkSignerEngine.SignerConfig>(mSignerConfigs.Count);
            ...
            Dictionary<String, CentralDirectoryRecord> outputCdRecordsByName =
                new Dictionary<string, CentralDirectoryRecord>(inputCdRecords.Count);
            foreach (CentralDirectoryRecord inputCdRecord in inputCdRecordsSortedByLfhOffset)
            {
                ...
                if (ApkUtils.SOURCE_STAMP_CERTIFICATE_HASH_ZIP_ENTRY_NAME.Equals(entryName))
                {
                    try
                    {
                ...
                InputJarEntryInstructions entryInstructions =
                ...
                if (shouldOutput)
                {
                    ...
                    if (pinPatterns != null)
                    {
                        ...
                        foreach (Hints.PatternWithRange pinPattern in pinPatterns)
                        {
                            if (pinPattern.matcher(inputCdRecord.getName()).Count > 0)
                            {
            ...
            List<CentralDirectoryRecord> outputCdRecords = new List<CentralDirectoryRecord>(inputCdRecords.Count + 10);
            foreach (CentralDirectoryRecord inputCdRecord in inputCdRecords)
            {
                String entryName = inputCdRecord.getName();
                outputCdRecordsByName.TryGetValue(entryName, out var outputCdRecord);
            ...
            if (signerEngine.isEligibleForSourceStamp())
            {
                ...
                if (mForceSourceStampOverwrite
                    ...
                    || uncompressedData.SequenceEqual(sourceStampCertificateDigest))
                {
                    outputOffset +=
                ...
                else
                {
                    throw new ApkFormatException(
                        String.Format(
                            ...
                            + " the name: {0}, and it is different than the provided source"
                            + " stamp certificate",
                            ApkUtils.SOURCE_STAMP_CERTIFICATE_HASH_ZIP_ENTRY_NAME));
                }
            }
            ...
            if (pinByteRanges != null)
            {
                ...
                pinByteRanges.Add(new Hints.ByteRange(outputOffset, long.MaxValue));
            ...
            OutputJarSignatureRequest outputJarSignatureRequest =
                signerEngine.outputJarEntries();
            if (outputJarSignatureRequest != null)
            {
                foreach (OutputJarSignatureRequestJarEntry entry in outputJarSignatureRequest.getAdditionalJarEntries())
                {
            ...
            foreach (CentralDirectoryRecord record in outputCdRecords)
            {
                outputCentralDirSizeBytes += record.getSize();
            }

            if (outputCentralDirSizeBytes > int.MaxValue)
            {
            ...
            int outputCentralDirRecordCount = outputCdRecords.Count;
        ...
        private class OutputSizeAndDataOffset
        {
        ...
        private static ByteBuffer getZipCentralDirectory(
            DataSource apk, ZipSections apkSections)
        ...
        private static List<CentralDirectoryRecord> parseZipCentralDirectory(
            ByteBuffer cd, ZipSections apkSections)

        {
            long cdOffset = apkSections.getZipCentralDirectoryOffset();
            int expectedCdRecordCount = apkSections.getZipCentralDirectoryRecordCount();
            List<CentralDirectoryRecord> cdRecords = new List<CentralDirectoryRecord>(expectedCdRecordCount);
            ISet<String> entryNames = new HashSet<string>(expectedCdRecordCount);
     ...
     */
        public static ByteBuffer getAndroidManifestFromApk(
     ...
     */
...

        {
            ...
            {
                pinPatterns = new List<Hints.PatternWithRange>();
                byte[] patternBlob;
                try
                {
                    patternBlob =
     ...
     */
...

        {
            ...
            catch (Exception e) when (e is ZipFormatException || e is ApkFormatException)
            {
     ...
     */
         ...
         * #setV1SigningEnabled(bool)}, {@link #setV2SigningEnabled(bool)}, {@link
         * #setOtherSignersSignaturesPreserved(bool)}, {@link #setCreatedBy(String)}.
         *
         ...
         */
            public Builder(ApkSignerEngine signerEngine)
            {
                ...
                {
                    throw new ArgumentNullException(nameof(signerEngine));
                }
         ...
         */
            public Builder setInputApk(FileInfo inputApk)
            {
                if (inputApk == null)
                {
                    throw new ArgumentNullException(nameof(inputApk));
                }
         ...
         */
            public Builder setOutputApk(FileInfo outputApk)
            {
                if (outputApk == null)
                {
                    throw new ArgumentNullException(nameof(outputApk));
                }
         ...
         */
            public Builder setOutputApk(DataSink outputApkOut, DataSource outputApkIn)
            {
                ...
                {
                    throw new ArgumentNullException(nameof(outputApkOut));
                }

                if (outputApkIn == null)
                {
                    throw new ArgumentNullException(nameof(outputApkIn));
                }
         ...
         * doesn't exist.
         */
            public Builder setV4SignatureOutputFile(FileInfo v4SignatureOutputFile)
            {
                if (v4SignatureOutputFile == null)
                {
                    throw new ArgumentNullException(nameof(v4SignatureOutputFile));
                }
         ...
         *
         * @throws InvalidOperationException if this builder was initialized with an {@link
         *     ApkSignerEngine}
         */
         ...
         */
            public Builder setV4SigningEnabled(bool enabled)
            {
                checkInitializedWithoutEngine();
         ...
         */
            public Builder setV4ErrorReportingEnabled(bool enabled)
            {
                checkInitializedWithoutEngine();
                mV4ErrorReportingEnabled = enabled;
                return this;
