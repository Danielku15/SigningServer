        ...
        public static List<SigningCertificateNode> readSigningCertificateLineage(ByteBuffer inputBytes)
        {
            ...
            catch (CryptographicException e)
            {
                throw new CryptographicException(
                    ...
                    + " when parsing V3SigningCertificateLineage object",
                    e);
            }
 ...
 */
            ...
            List<SigningCertificateNode> signingCertificateLineage)
        {
            ...
            foreach (SigningCertificateNode node in signingCertificateLineage)
            {
                nodes.Add(encodeSigningCertificateNode(node));
            }

            byte[] encodedSigningCertificateLineage =
                ApkSigningBlockUtils.encodeAsSequenceOfLengthPrefixedElements(nodes);
        ...
        public static byte[] encodeSignedData(X509Certificate certificate, int flags)
        {
            try
            {
                byte[] prefixedCertificate =
                    ApkSigningBlockUtils.encodeAsLengthPrefixedElement(certificate.getEncoded());
        ...
        public class SigningCertificateNode : IEquatable<SigningCertificateNode>
        {
            ...
            public bool Equals(SigningCertificateNode other)
            {
                ...
                return Equals(signingCert, other.signingCert) && Equals(parentSigAlgorithm, other.parentSigAlgorithm) &&
                       Equals(sigAlgorithm, other.sigAlgorithm) && signature.SequenceEqual(other.signature) &&
                       flags == other.flags;
            }
