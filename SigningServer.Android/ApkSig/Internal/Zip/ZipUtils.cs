...
namespace SigningServer.Android.ApkSig.Internal.Zip
{
    ...
    public static class ZipUtils
    {
        ...
        private static readonly int ZIP_EOCD_REC_MIN_SIZE = 22;
        private static readonly int ZIP_EOCD_REC_SIG = 0x06054b50;
        private static readonly int ZIP_EOCD_CENTRAL_DIR_TOTAL_RECORD_COUNT_OFFSET = 10;
        private static readonly int ZIP_EOCD_CENTRAL_DIR_SIZE_FIELD_OFFSET = 12;
        private static readonly int ZIP_EOCD_CENTRAL_DIR_OFFSET_FIELD_OFFSET = 16;
        private static readonly int ZIP_EOCD_COMMENT_LENGTH_FIELD_OFFSET = 20;

        private static readonly int UINT16_MAX_VALUE = 0xffff;
         ...
         */
        public static Tuple<ByteBuffer, long> findZipEndOfCentralDirectoryRecord(DataSource zip)
        {
            ...
            // reading more data.
            Tuple<ByteBuffer, long> result = findZipEndOfCentralDirectoryRecord(zip, 0);
            if (result != null)
            {
                return result;
            }
     ...
     * @throws IOException if an I/O error occurs while reading the file.
     */
        private static Tuple<ByteBuffer, long> findZipEndOfCentralDirectoryRecord(
            DataSource zip, int maxCommentSize)
        {
            // ZIP End of Central Directory (EOCD) record is located at the very end of the ZIP archive.
            // The record can be identified by its 4-byte signature/magic which is located at the very
            // beginning of the record. A complication is that the record is variable-length because of
            ...
            {
                throw new ArgumentException("maxCommentSize: " + maxCommentSize);
            }
     ...
     */
        ...
        public static void setUnsignedInt16(ByteBuffer buffer, int offset, int value)
        {
            if ((value < 0) || (value > 0xffff))
            {
                throw new ArgumentOutOfRangeException(nameof(value), value, "uint16 value of out range");
            }

            buffer.putShort(offset, (short)value);
        }

        public static void setUnsignedInt32(ByteBuffer buffer, int offset, long value)
        {
            if ((value < 0) || (value > 0xffffffffL))
            {
                throw new ArgumentOutOfRangeException(nameof(value), value, "uint32 of out range");
            }
        ...
        public static void putUnsignedInt16(ByteBuffer buffer, int value)
        {
            if ((value < 0) || (value > 0xffff))
            {
                throw new ArgumentOutOfRangeException(nameof(value), value, "uint16 of out range");
            }
        ...
        public static DeflateResult deflate(ByteBuffer input)
        {
            ...
            crc32.Update(new ArraySegment<byte>(inputBuf, inputOffset, inputLength));
            long crc32Value = crc32.Value;
            ...
            return new DeflateResult(inputLength, crc32Value, @out.ToArray());
        }
